...
def rot_matrix(ax, ay, az):
    ...
    cz = math.cos(az)
    sz = math.sin(az)
    Rx = ((1, 0, 0), (0, cx, -sx), (0, sx, cx))  # <-- fixed
...
def project(p):
    ...
    ooz = 1.0 / zc
    xp = int(W / 2 + K1 * x * ooz)
    yp = int(H / 2 - K1 * y * ooz)  # flip Y so +y goes up
    return xp, yp, ooz
...
def rasterize_triangle(output, zbuf, P, ch, oozs):
    (x0, y0), (x1, y1), (x2, y2) = P
    o0, o1, o2 = oozs
...
def main():
    ...
    while True:
        ...
        # Rotate to camera space (translation via +K2 in project)
        ...
        for i0, i1, i2 in F:
            ...
            # Backface culling
            n = cross(sub(B, A), sub(C, A))  # not normalized
            # Keep faces whose normal points toward camera:
            if n[2] >= 0:
                continue
